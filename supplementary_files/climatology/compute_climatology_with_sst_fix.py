...
def main(cfg: DictConfig):
    ...
    for idx_in_year in range(cfg.dataset.samples_per_file):
        ...
        # Fix the sst channel before taking the mean
        temp_container[:, 20] = np.where(temp_container[:, 20] == -32767.0, temp_container[:, 2], temp_container[:, 20])
    ...
    with h5py.File(f"{output_path}/climatology_with_sst_fix.h5", "w") as f:
