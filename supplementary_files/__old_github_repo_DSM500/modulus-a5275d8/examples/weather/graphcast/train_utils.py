...
def prepare_input(
    ...
    if cos_zenith is not None:
        ...
        cos_zenith = (cos_zenith - cos_zenith.mean()) / cos_zenith.std()
        invar = torch.concat(
    ...
    if time_idx is not None:
        ...
        for i in range(num_history + 1):
            ...
            sin_time_of_day[0, i] = torch.sin(torch.tensor((2 * np.pi * time_of_day) / 24))
            #cos_time_of_day[0, i] = torch.cos(normalized_time_of_day)
            cos_time_of_day[0, i] = torch.cos(torch.tensor((2 * np.pi * (time_of_day + dt)) / 24))

        # Standardize the clock variables
        sin_day_of_year = (sin_day_of_year - 0.6217995408581125) / 0.30581369116420254
        cos_day_of_year = (cos_day_of_year - 0.6604384742677212) / 0.28924783507700846
        sin_time_of_day = sin_time_of_day / 0.7071067811865478
        cos_time_of_day = cos_day_of_year / 0.7071067811865476
