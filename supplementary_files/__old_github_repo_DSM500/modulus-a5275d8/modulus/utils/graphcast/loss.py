...
class GraphCastLossFunction(nn.Module):
    ...
    def __init__(self, area=None, channels_list=None, dataset_metadata_path=None, time_diff_std_path=None, dtype=None, device=None):
        super().__init__()
        #self.area = area
        ...
        # Precompute, cast and move to device
        #self.time_diff_std = torch.square(self.time_diff_std.view(1, -1, 1, 1).to(device).to(dtype))
        #self.variable_weights = self.variable_weights.view(1, -1, 1, 1).to(device).to(dtype)
        #self.area = self.area.to(dtype) # TODO: Clean up for good if decided not to use this anymore
    ...
    def forward(self, invar, outvar):
        ...
        #loss /= self.time_diff_std

        # weighted by variables
        #loss = loss * self.variable_weights  # [T,C,H,W]
    ...
    def get_channel_dict(self, dataset_metadata_path, channels_list):
        ...
        with open(dataset_metadata_path, "r") as f:
            ...
            channel_dict = {"original_order": channel_list, "surface": [], "atmosphere": []}
    ...
    def assign_variable_weights(self):
        ...
        for channel in self.channel_dict["original_order"]:
            if channel in surface_weights_dict:
                variable_weights.append(surface_weights_dict[channel])
            else:
                variable_weights.append(atmosphere_weights_dict[channel])
        return torch.tensor(variable_weights)
