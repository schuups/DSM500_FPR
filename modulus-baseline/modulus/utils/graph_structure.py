...
from modulus.utils.icosahedral_mesh import (
    get_hierarchy_of_triangular_meshes_for_sphere,
    faces_to_edges,
    merge_meshes
)

from modulus.utils.graph_utils import (
    create_graph,
    create_heterograph,
    max_edge_length,
    xyz2latlon,
...
from modulus.utils.caching import Cache, MeshesCacheGuard
...
class Graph:
    def __init__(
        ...
        if Cache.is_initialized():
            _list_of_meshes = Cache.load(Cache.MESHES, guards=[
                MeshesCacheGuard(list_length=mesh_level+1)
            ])
        else:
            _list_of_meshes = None

        # ... or build it anew
        if _list_of_meshes is None:
            _list_of_meshes = get_hierarchy_of_triangular_meshes_for_sphere(mesh_level=mesh_level)
            if Cache.is_initialized():
                cache.store(Cache.MESHES, _list_of_meshes)


        _finest_mesh = _list_of_meshes[-1]
        if use_multimesh:
            _mesh = merge_meshes(_list_of_meshes)
        else:
            _mesh = _finest_mesh

        # The indices of the vertices forming an edge, e.g. torch.Size([327680, 2]) with mesh_level=6, torch.Size([245760, 2]) otherwise
        _src, _dst = faces_to_edges(_mesh.faces)
        self.src = torch.tensor(_src, dtype=torch.int32)
        self.dst = torch.tensor(_dst, dtype=torch.int32)
        # The indices of the vertices forming a face, e.g. torch.Size([109220, 3]) with mesh_level=6, torch.Size([81920, 3]) otherwise
        self.faces = torch.tensor(_mesh.faces, dtype=torch.int32)
        # The 3D coords of vertices, e.g. (40962, 3)
        self.vertices_3d = torch.tensor(_mesh.vertices, dtype=torch.float32)
        # The 2D equivalent of the 3D coords, e.g. (40962, 2)
        self.vertices_latlon = xyz2latlon(self.vertices_3d)

        # Get max edge length for the finest mesh, e.g. 0.020673068 for 7 mesh levels (splits=6)
        _src_inest_mesh, _dst_finest_mesh = faces_to_edges(_finest_mesh.faces)
        self.max_edge_len = max_edge_length(
            vertices=_finest_mesh.vertices,
            source_nodes=_src_inest_mesh,
            destination_nodes=_dst_finest_mesh
        )
    ...
    def create_m2m_graph(self):
        ...
        m2m_graph.edata["displacement_info"] = self._compute_edge_displacement(
            graph=m2m_graph, 
            src_3D_coords=self.vertices_3d, 
            dst_3D_coords=self.vertices_3d
        )
        return m2m_graph

    def create_g2m_graph(self):
        """Create the grid2mesh bipartite graph."""
        src, dst = self._get_g2m_edges(self.list_grid_3D_coords, self.vertices_3d, self.max_edge_len)
    ...
    def create_m2g_graph(self):
        ...
        src, dst = self._get_m2g_edges(self.vertices_3d, self.faces, self.list_grid_3D_coords)
